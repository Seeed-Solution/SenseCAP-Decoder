/**
 * Payload Decoder for Datacake https://datacake.co/
 * based on https://github.com/Seeed-Solution/TTN-Payload-Decoder/blob/master/SenseCAP_LoRaWAN_V2_Decoder_For_TTN.js
 *
 * @product SenseCAP S2100 LoRaWAN Data Logger
 * @author Datacake GmbH
 * @license Unlicensed for internal use
 */
...
function Decoder(bytes, port) {
  ...
  // Array to store the fields that are being sent to Datacake
  var datacakeFields = [];

  // take each field from decodedSensecapFields and convert them to Datacake format
  for (var i = 0; i < decodedSensecapFields.length; i++) {
    var arrayEntry = decodedSensecapFields[i];
    for (var e = 0; e < arrayEntry.length; e++) {
      if (arrayEntry[e].hasOwnProperty('Battery Percentage')) {
        for (var key in arrayEntry[e]) {
          if (arrayEntry[e].hasOwnProperty(key)) {
            datacakeFields.push({
              field: key.toUpperCase().replace(new RegExp(' ', 'g'), '_'),
              value: arrayEntry[e][key],
            });
          }
        }
      } else {
        datacakeFields.push({
          field: arrayEntry[e]['type']
              .toUpperCase()
              .replace(new RegExp(' ', 'g'), '_'),
          value: arrayEntry[e]['measurementValue'],
        });
      }
    }
  }
  ...
  if (normalizedPayload.gateways && normalizedPayload.gateways[0]) {
    var loraRSSI = normalizedPayload.gateways[0].rssi;
    var loraDatarate = normalizedPayload.data_rate;
    var loraSNR = normalizedPayload.gateways[0].snr;
    datacakeFields.push({ field: 'LORA_RSSI', value: loraRSSI });
    datacakeFields.push({ field: 'LORA_DATARATE', value: loraDatarate });
    datacakeFields.push({ field: 'LORA_SNR', value: loraSNR });
  }
...
function decodeSensecapPayload(data) {
  ...
  data = data2HexString(data).toLocaleUpperCase();
  var result = {
    err: 0,
    payload: data,
    valid: true,
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    switch (dataId) {
      ...
      case '45':
        ...
        data = remainingValue.substring(22);
        dataObj = {
          dataId: dataId,
          dataValue: dataValue,
        };
      ...
      case '02':
        ...
        data = remainingValue.substring(18);
        dataObj = {
          dataId: '02',
          dataValue: dataValue,
        };
      ...
      case '06':
        ...
        data = remainingValue.substring(4);
        dataObj = {
          dataId: dataId,
          dataValue: dataValue,
        };
      ...
      case '05':
      case '34':
        dataValue = data.substring(2, 10);
        data = remainingValue.substring(10);
        dataObj = {
          dataId: dataId,
          dataValue: dataValue,
        };
      ...
      case '38':
      case '39':
        dataValue = data.substring(2, 20);
        data = remainingValue.substring(20);
        dataObj = {
          dataId: dataId,
          dataValue: dataValue,
        };
...
function dataIdAndDataValueJudge(dataId, dataValue) {
  ...
  switch (dataId) {
    ...
    case '03':
      ...
      messages = [
        {
          'Battery Percentage': loraWANV2DataFormat(Electricity),
        },
      ];
      break;
    case '04':
      ...
      messages = [
        {
          'Battery Percentage': loraWANV2DataFormat(electricityWhether),
          'Hardware Version': ''
              .concat(loraWANV2DataFormat(hwv.substring(0, 2)), '.')
              .concat(loraWANV2DataFormat(hwv.substring(2, 4))),
          'Firmware Version': ''
              .concat(loraWANV2DataFormat(bdv.substring(0, 2)), '.')
              .concat(loraWANV2DataFormat(bdv.substring(2, 4))),
          measureInterval:
    ...
    case '06':
      ...
      switch (errorCode) {
        ...
        case '15':
          ...
          break;
        case 'FF':
          descZh = 'CCL_SENSOR_DATA_ERROR_UNKNOWN';
          break;
        default:
    ...
    case '20':
      ...
      for (var i = 0; i < dataValue.length; i += 4) {
        ...
        var detectionType = loraWANV2DataFormat(
    ...
    case '21':
      // Vision AI:
      // AI identification of output frames
      var tailValueArray = [];
      ...
      for (var _i = 0; _i < dataValue.length; _i += 4) {
        ...
        var _detectionType = loraWANV2DataFormat(
            dataValue.substring(_i + 2, _i + 4)
        );
    ...
    case '30':
    case '31':
      // First frame or first output frame
      ...
      var dataOne = {
        ...
        type: getDataLoggerMeasurementName(channelInfoOne.one),
      };
      var dataTwo = {
        measurementValue: loraWANV2DataFormat(
            dataValue.substring(12, 20),
            1000
        ),
        measurementId: parseInt(channelInfoOne.two),
        type: getDataLoggerMeasurementName(channelInfoOne.two),
      };
      var cacheArrayInfo = [];
    ...
    case '32':
      ...
      var dataThree = {
        ...
        type: getDataLoggerMeasurementName(channelInfoTwo.one),
      };
      var dataFour = {
        measurementValue: loraWANV2DataFormat(
            dataValue.substring(10, 18),
            1000
        ),
        measurementId: parseInt(channelInfoTwo.two),
        type: getDataLoggerMeasurementName(channelInfoTwo.two),
      };
      ...
      break;
    case '33':
      var channelInfoThree = loraWANV2ChannelBitFormat(
          dataValue.substring(0, 2)
      );
      var dataFive = {
        measurementValue: loraWANV2DataFormat(dataValue.substring(4, 12), 1000),
        measurementId: parseInt(channelInfoThree.one),
        type: getDataLoggerMeasurementName(channelInfoThree.one),
      };
      var dataSix = {
        measurementValue: loraWANV2DataFormat(
            dataValue.substring(12, 20),
        ...
        type: getDataLoggerMeasurementName(channelInfoThree.two),
      };
      ...
      messages = [
        ((_ref = {}),
            _defineProperty(
                _ref,
                channelSortTDOne,
                loraWANV2DataFormat(dataValue.substring(2, 10), 1000)
            ),
            _defineProperty(
                _ref,
                channelSortTDTWO,
                loraWANV2DataFormat(dataValue.substring(10, 18), 1000)
            ),
            _ref),
      ];
      break;
    case '37':
      var channelTDInfoTwo = loraWANV2ChannelBitFormat(
          dataValue.substring(0, 2)
      );
      ...
      break;
    case '38':
      var channelTDInfoThree = loraWANV2ChannelBitFormat(
          dataValue.substring(0, 2)
      );
      var channelSortThreeOne =
          3920 + (parseInt(channelTDInfoThree.one) - 1) * 2;
      var channelSortThreeTWO =
    ...
    case '39':
      ...
      messages = [
        {
          'Battery Percentage': loraWANV2DataFormat(electricityWhetherTD),
          'Hardware Version': ''
              .concat(loraWANV2DataFormat(hwvTD.substring(0, 2)), '.')
              .concat(loraWANV2DataFormat(hwvTD.substring(2, 4))),
          'Firmware Version': ''
              .concat(loraWANV2DataFormat(bdvTD.substring(0, 2)), '.')
              .concat(loraWANV2DataFormat(bdvTD.substring(2, 4))),
          measureInterval:
              parseInt(loraWANV2DataFormat(sensorAcquisitionIntervalTD)) * 60,
          thresholdMeasureInterval: parseInt(
              loraWANV2DataFormat(gpsAcquisitionIntervalTD)
          ),
        },
      ];
      break;
    ...
    case '44':
      ...
      for (var _i2 = 0; _i2 < dataValue.length; _i2 += 4) {
        ...
        var _detectionType2 = loraWANV2DataFormat(
            dataValue.substring(_i2 + 2, _i2 + 4)
        );
    ...
    case '45':
      ...
      for (var _i3 = 0; _i3 < dataValue.length; _i3 += 4) {
        ...
        var _detectionType3 = loraWANV2DataFormat(
            dataValue.substring(_i3 + 2, _i3 + 4)
        );
...
function getDataLoggerMeasurementName(num) {
  var letters = {
    1: 'One',
    2: 'Two',
    3: 'Three',
